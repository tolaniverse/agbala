...
// The visual design, written out by hand.
//
// These structs were authored from the mock before any protocol existed, so
// they owe the schema nothing — which is exactly what makes them worth keeping.
// They are the specification the fold is measured against in equal_test.go: if
// folding a log cannot arrive here, the protocol is missing something.
//
// They live in a test rather than in internal/scene because the package now
// folds the committed fixtures like any other client. Keeping the hand-written
// version here is what stops that from being a tautology.
...
// designName identifies one of the designed states.
type designName string

// The states the design specifies, in the order it presents them.
const (
	designBoot     designName = "boot"
	designLoop     designName = "loop"
	designDeny     designName = "deny"
	designApproval designName = "approval"
	designFork     designName = "fork"
)

// designNames lists every designed state.
func designNames() []designName {
	return []designName{designBoot, designLoop, designDeny, designApproval, designFork}
}

// handwritten returns the design's own rendering of a state.
func handwritten(n designName) (ui.Screen, bool) {
	build, ok := map[designName]func() ui.Screen{
		designBoot: boot, designLoop: loop, designDeny: deny,
		designApproval: approval, designFork: fork,
	}[n]
	if !ok {
...
func statusBar(state string, tone theme.Tone, turn string) ui.Session {
	return ui.Session{
...
func composer(loading string, mode theme.Mode, dirty string) ui.Input {
	return ui.Input{
		Loading: loading,
...
func boot() ui.Screen {
	...
	return ui.Screen{
		Session: statusBar("PLANNING", theme.ToneAgent, "1"),
		Rail:    rail,
		Input:   composer("resolving òfin scope · 1.2s", theme.ModePlan, "clean"),
		Blocks: []ui.Block{
...
func loop() ui.Screen {
	return ui.Screen{
		Session: statusBar("EXECUTING", theme.ToneOK, "14"),
		Rail:    baseRail(),
		Input:   composer("go test ./...  ·  6s  ·  esc to interrupt", theme.ModeAuto, "3 changed"),
		Blocks: []ui.Block{
...
func deny() ui.Screen {
	...
	return ui.Screen{
		Session: statusBar("PLANNING", theme.ToneAgent, "22"),
		Rail:    rail,
		Input:   composer("re-planning under ofin-014 · 2.4s", theme.ModeAuto, "clean"),
		Blocks: []ui.Block{
...
func approval() ui.Screen {
	...
	in := composer("turn suspended · waiting on you", theme.ModeAuto, "clean")
	...
	return ui.Screen{
		Session: statusBar("AWAITING_APPROVAL", theme.ToneHuman, "31"),
		Rail:    rail,
		Input:   in,
		Blocks: []ui.Block{
...
func fork() ui.Screen {
	...
	return ui.Screen{
		Session: statusBar("EXECUTING", theme.ToneFork, "45"),
		Rail:    rail,
		Input:   composer("3 sandboxes running · 1 still working", theme.ModeAuto, "clean"),
		Blocks: []ui.Block{
